...
#pragma omp parallel for if (results.size() > 10)
                                          ...
                                          int maxDisplay)
    {
        ...
        int outputWidth = tileSize * (numToShow + 2) + padding * (numToShow + 3);
        int outputHeight = tileSize + 2 * padding + 60;
        cv::Mat output(outputHeight, outputWidth, CV_8UC3, cv::Scalar(255, 255, 255));
        ...
        int queryX = padding * 2 + tileSize;
        cv::Mat queryVis(tileSize, tileSize, CV_8UC3, cv::Scalar(255, 255, 255));
        auto queryContour = cssComputer_.extractContour(queryImage);

        if (!queryContour.empty())
        {
            cv::Rect bbox = cv::boundingRect(queryContour);
            ...
            cv::polylines(queryVis, scaled, true, cv::Scalar(0, 0, 0), 2);
        }

        cv::Rect queryRoi_C(queryX, padding, tileSize, tileSize);
        queryVis.copyTo(output(queryRoi_C));
        cv::putText(output, "Detected Contour", cv::Point(queryX + 10, tileSize + padding + 25),
                    cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 0), 2);

        // Draw matches
        for (int i = 0; i < numToShow; i++)
        {
            int x = padding * (i + 2 + 1) + tileSize * (i + 1 + 1);

            cv::Mat matchImg(tileSize, tileSize, CV_8UC3, cv::Scalar(255, 255, 255));
            ...
            if (!contour.empty())
            {
                ...
                cv::polylines(matchImg, scaled, true, cv::Scalar(0, 0, 0), 2);
            }
